...
import { requestReset, resetPassword } from "@/store/global/actions";
import { z } from "zod";
				...
				<form onSubmit={form.handleSubmit(onSubmit)} className="w-full p-4">
					<Card className="p-8 space-y-6 text-white bg-grey-dark rounded-5 border-grey-light">
						...
						<CardContent className="flex flex-col gap-6 p-0 text-left ">
							{!isLoading && message === "success" ? <CardDescription className="text-base text-center">
						...
						<CardFooter className="flex flex-col p-0">
							{!isLoading && message === "success" ? (
								<Link to="/login" className="w-full h-10 px-4 py-2 font-medium text-center rounded-md bg-primary text-primary-foreground hover:bg-primary/90">{t("login")}</Link>
							) : (
								<>
									<Button
